"use client";

import { motion } from 'framer-motion';

const Testimonials = () => {
  const testimonials = [
    {
      content: "The virtual assistance services have transformed how we manage our practice. Our administrative efficiency has improved significantly.",
      author: "Dr. Sarah Johnson",
      role: "Family Physician",
    },
    {
      content: "Exceptional service! The team handles our patient scheduling and records with utmost professionalism and attention to detail.",
      author: "Dr. Michael Chen",
      role: "Pediatrician",
    },
    {
      content: "Their medical transcription service is outstanding. Fast, accurate, and always reliable.",
      author: "Dr. Emily Rodriguez",
      role: "Cardiologist",
    },
  ];

  return (
    <section className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center">
          <h2 className="text-3xl font-medium text-[#2D3748] sm:text-4xl">
            What Our Clients Say
          </h2>
          <p className="mt-4 max-w-2xl mx-auto text-xl text-gray-500">
            Trusted by healthcare professionals across the country
          </p>
        </div>
        
        <div className="mt-20 grid grid-cols-1 gap-8 lg:grid-cols-3">
          {testimonials.map((testimonial, index) => (
            <div
              key={index}
              className="relative bg-gray-50 p-8 rounded-lg hover:shadow-md transition-shadow duration-300"
            >
              <div className="relative">
                <svg
                  className="h-12 w-12 text-[#FF8873] absolute top-0 left-0 transform -translate-x-6 -translate-y-8 opacity-20"
                  fill="currentColor"
                  viewBox="0 0 32 32"
                >
                  <path d="M9.352 4C4.456 7.456 1 13.12 1 19.36c0 5.088 3.072 8.064 6.624 8.064 3.36 0 5.856-2.688 5.856-5.856 0-3.168-2.208-5.472-5.088-5.472-.576 0-1.344.096-1.536.192.48-3.264 3.552-7.104 6.624-9.024L9.352 4zm16.512 0c-4.8 3.456-8.256 9.12-8.256 15.36 0 5.088 3.072 8.064 6.624 8.064 3.264 0 5.856-2.688 5.856-5.856 0-3.168-2.304-5.472-5.184-5.472-.576 0-1.248.096-1.44.192.48-3.264 3.456-7.104 6.528-9.024L25.864 4z" />
                </svg>
                <p className="relative text-lg text-gray-600">{testimonial.content}</p>
              </div>
              <div className="mt-8">
                <div className="font-medium text-[#2D3748]">{testimonial.author}</div>
                <div className="text-[#FF8873]">{testimonial.role}</div>
              </div>
              <div className="absolute inset-0 border-2 border-transparent hover:border-[#FF8873] rounded-lg transition-colors duration-300" />
            </div>
          ))}
        </div>
      </div>
    </section>
  );
};

export default Testimonials;
